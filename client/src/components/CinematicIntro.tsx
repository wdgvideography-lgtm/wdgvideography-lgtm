 ...
 * Hardened: unmount guard, max-wait failsafe (15s), logo onError, RAF cleanup
 */
...
export default function CinematicIntro({ onComplete }: CinematicIntroProps) {
  ...
  // Track video time → update overlays + progress bar
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (video.ended || (video.duration > 0 && time >= video.duration - 0.5)) {
        handleComplete();
        return;
      }
  ...
  // Start video + failsafe timeout
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;

    // Failsafe: if video hasn't started within MAX_WAIT_MS, skip the intro
    const failsafe = setTimeout(() => {
      if (mountedRef.current && !videoReady) handleComplete();
    }, MAX_WAIT_MS);
    ...
    <AnimatePresence>
      {!isExiting ? (
        ...
        >
          ...
          {autoplayBlocked && (
            <div
              className="absolute inset-0 flex flex-col items-center justify-center z-50 cursor-pointer bg-black/50"
              onClick={() => {
            ...
            >
              <div className="w-24 h-24 rounded-full border-2 border-gold/60 flex items-center justify-center bg-black/60 backdrop-blur-sm hover:bg-gold/20 transition-all duration-300">
                <svg className="w-10 h-10 text-gold ml-2" fill="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="absolute bottom-[8vh] right-6 sm:right-10 z-50 flex items-center gap-2 px-5 py-2.5 text-[10px] font-body text-white/50 tracking-[0.25em] uppercase hover:text-white transition-all duration-300 border border-white/10 rounded-sm hover:border-gold/50 hover:bg-gold/5 backdrop-blur-sm"
          >
            Skip
