    ...
    <section id="marketing" ref={sectionRef} className="relative py-28 lg:py-36 overflow-hidden">
      ...
      <div className="container">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <div>
            <motion.span initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="inline-block text-xs font-body text-gold tracking-[0.3em] uppercase mb-4">Full-Service</motion.span>
            <motion.h2 initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.1 }} className="font-display text-4xl md:text-5xl font-bold text-foreground mb-8 leading-[1.1]">
              Marketing Management{" "}<span className="text-gradient-gold">Services</span>
            </motion.h2>
            <motion.p initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.2 }} className="text-muted-foreground font-body text-lg leading-relaxed mb-4">
            ...
            <motion.p initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.3 }} className="text-muted-foreground/80 font-body leading-relaxed mb-10">
              We don't just post; we strategise, tailoring our services and providing real results for your business.
            </motion.p>
            <motion.a initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.4 }} href="/contact?service=social-media" className="inline-flex items-center gap-3 px-7 py-3.5 bg-gold text-primary-foreground font-body font-semibold text-sm tracking-wide rounded-sm hover:bg-gold-light transition-all duration-500 hover:shadow-[0_0_30px_oklch(0.78_0.12_75/0.4)] group">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-5">
            {marketingServices.map((service, index) => (
              <motion.div key={service.title} initial={{ opacity: 0, y: 50, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.7, delay: index * 0.12, ease: [0.22, 1, 0.36, 1] }} whileHover={{ y: -6, scale: 1.03, transition: { duration: 0.3 } }} className="relative p-6 lg:p-7 rounded-sm border border-border/40 bg-card/30 backdrop-blur-sm hover:border-gold/30 hover:bg-card/50 transition-all duration-500 group overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-gold/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className="relative z-10">
                  <div className="w-10 h-10 rounded-sm border border-gold/20 bg-gold/5 flex items-center justify-center text-gold mb-5 group-hover:border-gold/40 group-hover:bg-gold/10 transition-all duration-300">{service.icon}</div>
